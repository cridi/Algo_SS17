...
public:
	...
	Impedanz(string Name, string Real, string Imag, string Pin1, string Pin2);
};
Impedanz::Impedanz(string a, string b, string c, string d, string e) {
	Name = a;
	Real = b;
	Imag = c;
	Pin1 = d;
...
vector<Impedanz*> Impedanzen;
vector<Impedanz*> serial_Impedanzen;
...
void find_serial_Bauteil(string serial_Pin) {
	...
	for (unsigned int i = 0; i < Impedanzen.size(); i++) {
		if ((!((Impedanzen.at(i)->Pin1).compare(serial_Pin))) || (!((Impedanzen.at(i)->Pin2).compare(serial_Pin)))) {
			serial_Impedanzen.push_back(Impedanzen.at(i));
			break;
		}
	}
}
...
vector<string*> find_outer_Pins(string serial_Pin) {
	vector<string*> outer_Pins;

	if ((serial_Pin.compare(serial_Bauteile.at(0)->Pin1))) {
		outer_Pins.push_back(&serial_Bauteile.at(0)->Pin1);
	}
	else outer_Pins.push_back(&serial_Bauteile.at(0)->Pin2);

	if ((serial_Pin.compare(serial_Bauteile.at(1)->Pin1))) {
		outer_Pins.push_back(&serial_Bauteile.at(1)->Pin1);
	}
	else outer_Pins.push_back(&serial_Bauteile.at(1)->Pin2);

	return outer_Pins;
}

void create_new_serial_Bauteil(vector<string*> outer_Pins) {
	...
	if (serial_Bauteile.at(0)->Art == serial_Bauteile.at(1)->Art) {
		...
		if (serial_Bauteile.at(0)->Art == "C") {
			Bauteile.push_back(new Bauteil(	serial_Bauteile.at(0)->Name + "||" + serial_Bauteile.at(1)->Name,
											serial_Bauteile.at(1)->Art, 
											*outer_Pins.at(0), *outer_Pins.at(1)));
		}
		else {
			Bauteile.push_back(new Bauteil(	"(" + serial_Bauteile.at(0)->Name + " + " + serial_Bauteile.at(1)->Name + ")",
											serial_Bauteile.at(1)->Art, 
											*outer_Pins.at(0), *outer_Pins.at(1)));				//New Element with outer Pins and new Name
		}
	...
	serial_Bauteile.clear();
}
void create_new_serial_Impedanz(vector<string*> outer_Pins) {
	vector<Impedanz*> RI_Impedanzen;

	for (unsigned int i = 0; i < serial_Bauteile.size(); i++) {
		if (serial_Bauteile.at(i)->Art == "R") {
			RI_Impedanzen.push_back(new Impedanz(	"Z_" + to_string(i),
													"+" + serial_Bauteile.at(i)->Name,
													"",
													serial_Bauteile.at(i)->Pin1, serial_Bauteile.at(i)->Pin2));
		}
		else if (serial_Bauteile.at(i)->Art == "L") {
			RI_Impedanzen.push_back(new Impedanz(	"Z_" + to_string(i),
													"",
													"+w*" + serial_Bauteile.at(i)->Name,
													serial_Bauteile.at(i)->Pin1, serial_Bauteile.at(i)->Pin2));
		}
		else if (serial_Bauteile.at(i)->Art == "C") {
			RI_Impedanzen.push_back(new Impedanz(	"Z_" + to_string(i),
													"",
													"-1/(w*" + serial_Bauteile.at(i)->Name + ")",
													serial_Bauteile.at(i)->Pin1, serial_Bauteile.at(i)->Pin2));
		}
		else if (serial_Bauteile.at(i)->Art == "Z") {
			for (int i = 0; i < Impedanzen.size(); i++) {
				if ((Impedanzen.at(i)->Name == serial_Bauteile.at(0)->Name) || (Impedanzen.at(i)->Name == serial_Bauteile.at(1)->Name)) {
					RI_Impedanzen.push_back(new Impedanz(	"Z_" + to_string(i),
															Impedanzen.at(i)->Real,
															Impedanzen.at(i)->Imag,
															Impedanzen.at(i)->Pin1, Impedanzen.at(i)->Pin2));
				}
			}
		}
	}

	for (int i = 0; i < serial_Impedanzen.size(); i++) {

			RI_Impedanzen.push_back(new Impedanz(	"Z_" + to_string(i),
													serial_Impedanzen.at(i)->Real,
													serial_Impedanzen.at(i)->Imag,
													serial_Impedanzen.at(i)->Pin1, serial_Impedanzen.at(i)->Pin2));
	}

	Bauteile.push_back(new Bauteil(	"Z" + to_string(Impedanzen.size() + 1), "Z",
									*outer_Pins.at(0), *outer_Pins.at(1)));				//New Element with outer Pins and new Name

	Impedanzen.push_back(new Impedanz(	"Z" + to_string(Impedanzen.size() + 1), 
										RI_Impedanzen.at(0)->Real + RI_Impedanzen.at(1)->Real,
										RI_Impedanzen.at(0)->Imag + RI_Impedanzen.at(1)->Imag,
										*outer_Pins.at(0), *outer_Pins.at(1)));				//New Element with outer Pins and new Name

	if (serial_Bauteile.size() == 2) {
		for (int i = 0; i < Bauteile.size(); i++) {
			if (!(serial_Bauteile.at(0)->Name.compare(Bauteile.at(i)->Name)) || (!(serial_Bauteile.at(1)->Name.compare(Bauteile.at(i)->Name)))) {
				Bauteile.erase(Bauteile.begin() + i);
				i = -1;
	...
	else if (serial_Bauteile.size() == 1) {
		for (int i = 0; i < Bauteile.size(); i++) {
			if (!(serial_Bauteile.at(0)->Name.compare(Bauteile.at(i)->Name))) {
				Bauteile.erase(Bauteile.begin() + i);
				i = -1;
			}
		}
	...
	if (serial_Impedanzen.size() == 2) {
		for (int i = 0; i < Impedanzen.size(); i++) {
			if (!(serial_Impedanzen.at(0)->Name.compare(Impedanzen.at(i)->Name)) || (!(serial_Impedanzen.at(1)->Name.compare(Impedanzen.at(i)->Name)))) {
				Impedanzen.erase(Impedanzen.begin() + i);
				i = -1;
			}
		}
	}
	else if (serial_Impedanzen.size() == 1) {
		for (int i = 0; i < Impedanzen.size(); i++) {
			if (!(serial_Impedanzen.at(0)->Name.compare(Impedanzen.at(i)->Name))) {
				Impedanzen.erase(Impedanzen.begin() + i);
				i = -1;
			}
		}
...
void Is_serial() {
	...
	for (int i = 0; i < serial_Pins.size(); i++) {
		...
		outer_Pins = find_outer_Pins(*serial_Pins.at(i));
		create_new_serial_Bauteil(outer_Pins);
	}

}
void Is_serialImpedanz() {
	...
	for (int i = 0; i < serial_Pins.size(); i++) {
		...
		reverse_find_serial_Bauteil(*serial_Pins.at(i));
		outer_Pins = find_outer_Pins(*serial_Pins.at(i));
		create_new_serial_Impedanz(outer_Pins);
	}
...
string BauteilInput() {
	...
	InputBuffer = "R1:R(a, d); R7:R(f, b); C1:C(d,f); C2:C(d,g); C3:C(f,g);";			// For Testing only 
	for (spacepos; spacepos < InputBuffer.size(); spacepos++) {
		if (isspace(InputBuffer[spacepos])) {
			spacepos++;
			if (spacepos == InputBuffer.size()) break;
...
void print_network() {
	for (unsigned int i = 0; i < Bauteile.size(); i++) {
		cout << Bauteile.at(i)->Name << ":" << Bauteile.at(i)->Art << "(" << Bauteile.at(i)->Pin1 << "," << Bauteile.at(i)->Pin2 << ")" << "\t";
	}
	cout << endl;
	for (unsigned int i = 0; i < Impedanzen.size(); i++) {
		cout << Impedanzen.at(i)->Name << "=" << Impedanzen.at(i)->Real << "+ j*" << Impedanzen.at(i)->Imag << "\t";
	}
}
